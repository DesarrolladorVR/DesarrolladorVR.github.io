...
let currentModel = 'full'; // 'full' o 'lite'
...
const canvasCtx = canvasElement.getContext("2d");const armsUpSound = document.getElementById("armsUpSound");
...
// ============================================================
// DETECCIÓN DE BRAZOS LEVANTADOS CON AUDIO FEEDBACK
// ============================================================
...
function stopCamera() {
  webcamRunning = false;
  if (cameraSection) cameraSection.classList.remove('camera-active');
  if (stopButton) stopButton.style.display = 'none';
  if (startCameraSection) startCameraSection.style.display = 'flex';
  if (webcamButton) webcamButton.style.display = 'inline-flex';
...
}// Agregar evento al botón de iniciar cámara
...
async function enableCam(event) {
  ...
  if (webcamRunning === true) {
    ...
    // Actualizar UI: ocultar botón de inicio, mostrar botón detener flotante
    if (startCameraSection) startCameraSection.style.display = 'none';
    if (cameraSection) cameraSection.classList.add('camera-active');
    if (webcamButton) webcamButton.style.display = 'none';
    ...
    try {
      ...
      video.addEventListener("loadeddata", () => {        // Ocultar loading cuando el video esté listo
        if (loadingOverlay) {
...
async function autoStartCamera() {
  ...
  const hintTimeout = setTimeout(() => {
    ...
    if (loadingHint && (!loadingOverlay || loadingOverlay.style.display !== 'none')) {
    ...
    if (poseLandmarker) {
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          // Ocultar loading y mostrar botón de inicio manual
          hideLoading();
          if (startCameraSection) startCameraSection.style.display = 'flex';
        }
      }, 500);
    } else if (attempts >= maxAttempts) {
      // Timeout: modelo no cargó
      clearInterval(checkModel);
      clearTimeout(hintTimeout);
      console.error('Timeout: Modelo no cargó en 5 segundos');
      hideLoading();
      if (startCameraSection) startCameraSection.style.display = 'flex';
      updateStatus('❌ Error al cargar modelo. Click para reintentar.');
    }
  }, 100);

  // Seguro extra: ocultar loading si algo se cuelga después de 7s
  setTimeout(() => {
    hideLoading();
    if (startCameraSection) startCameraSection.style.display = 'flex';
  }, 7000);
...
if (applyModelButton) {
  applyModelButton.addEventListener('click', async () => {
    ...
    if (wasRunning) {
      updateStatus('Modelo cambiado. Reactivando cámara...');
      setTimeout(() => {
        webcamButton.click();
      }, 500);
    } else {
...
console.log('%c Sistema de Optimización de Rendimiento Activado', 
  'color: #00FF88; font-size: 12px; font-weight: bold;');
console.log(`%c Resolución optimizada: 960x540 | Frame skip: 1 de cada 2`, 
